#!/bin/bash
# shotlist-studio 一条命令管线：run / stop / status / test / export / seed
...
case "$CMD" in
  ...
  help)    echo "用法: bash scripts/studio.sh {run|stop|status|test|export|seed} [参数]" ;;
  *)       echo "用法: bash scripts/studio.sh {run|stop|status|test|export|seed} [参数]" >&2; exit 2 ;;
esac
